...
| `PSMUX_FORCE_WHEEL` | Set to `1` to forward the wheel into every pane regardless of whether its application registered for the mouse. psmux normally forwards only to a pane that asked, either by sending a mouse DECSET or by having `ENABLE_MOUSE_INPUT` on its console. Console input mode belongs to the console rather than to the process that set it, so any child that enters raw mode — a `node` subprocess is the common case — wipes that flag and does not put it back, and the pane's wheel goes silent until you open a new window. Set this if a full screen application that reads the wheel perfectly well stops scrolling partway through a session. Leave it unset if you run programs that do not read the mouse: they receive the report as literal keystrokes, which is what the gate exists to prevent. Accepts `1`, `on`, `true`, `yes`; anything else keeps the gate |
